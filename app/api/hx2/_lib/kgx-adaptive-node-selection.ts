...
export async function buildKgxAdaptiveNodeSelection(userRequest: string) {
  ...
  const combinationFailureInfluence =
    await buildKgxCombinationFailureInfluence(userRequest);
  const netCombinationInfluence =
  ...
  const predictionTrustTrace: any[] = [];

  const trustScore =
    Number(predictionTrust.trust_score || 0);
  ...
  for (const node of predictedNodes) {
    const before = scores[node] || 0;
    const after =
      Math.round(before * trustMultiplier * 10) / 10;

    predictionTrustTrace.push({
      node,
  ...
  if (
    bestRoutingAssembly?.assembly_key &&
    bestRoutingAssembly?.routing_decision_score > 0
  ) {
    const routingBoost =
      Math.round(
        Math.min(
          Number(bestRoutingAssembly.routing_decision_score || 0) / 20,
          20
        ) * 10
      ) / 10;

    const routingNodes =
      String(bestRoutingAssembly.assembly_key)
        .split("+")
        .map(x => x.trim())
        .filter(Boolean);
    ...
    for (const node of routingNodes) {
      const before = scores[node] || 0;
      const after = before + routingBoost;

      routingDecisionConsumptionTrace.push({
        node,
        before: Math.round(before * 10) / 10,
        routing_boost: routingBoost,
        after: Math.round(after * 10) / 10
      });
  ...
  const netCombinationInfluenceTrace: any[] = [];

  if (
    netCombinationInfluence.recommended_assembly &&
    netCombinationInfluence.net_influence !== 0
  ) {
    const netNodes =
      String(netCombinationInfluence.recommended_assembly)
        .split("+")
        .map(x => x.trim())
        .filter(Boolean);
    ...
    for (const node of netNodes) {
      const before = scores[node] || 0;
      const net = Number(netCombinationInfluence.net_influence || 0);
      const after = Math.max(0, before + net);

      netCombinationInfluenceTrace.push({
        node,
        before: Math.round(before * 10) / 10,
        net_influence: net,
        after: Math.round(after * 10) / 10
  ...
  const combinationFailureTrace: any[] = [];

  if (
    combinationFailureInfluence.found &&
    combinationFailureInfluence.best_assembly &&
    combinationFailureInfluence.penalty > 0
  ) {
    const failureNodes =
      String(combinationFailureInfluence.best_assembly)
        .split("+")
        .map(x => x.trim())
        .filter(Boolean);
    ...
    for (const node of failureNodes) {
      const before = scores[node] || 0;
      const penalty = Number(combinationFailureInfluence.penalty || 0);
      const after = Math.max(0, before - penalty);

      combinationFailureTrace.push({
        node,
        before: Math.round(before * 10) / 10,
        penalty,
        after: Math.round(after * 10) / 10
  ...
  return {
    ...
    combination_failure_influence: combinationFailureInfluence,
    combination_failure_trace: combinationFailureTrace,
    net_combination_influence: netCombinationInfluence,
