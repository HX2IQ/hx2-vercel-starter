...
export default function Page() {
  ...
  const [payloadText, setPayloadText] = useState(JSON.stringify({"query":"Childcraft crib hardware kit","model":"F09523"}, null, 2));
  const [busy, setBusy] = useState(false);
  const [result, setResult] = useState<any>(null);

  const taskType = "shop.find";
  ...
  return (
    <main style={{ maxWidth: 980, margin: "0 auto", padding: 16, fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, Arial" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", gap:12 }}>
        <h1 style={{ fontSize: 28, fontWeight: 900, marginBottom: 6 }}>Shop</h1>
